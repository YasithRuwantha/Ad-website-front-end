...
export default function EarningsPage() {
  ...
  const handleUpgradePlan = (plan: "starter" | "normal" | "premium", price: number) => {
    if (user && user.balance >= price) {
      const newBalance = user.balance - price
      updateUser({ plan, balance: newBalance })
          ...
          <CardContent className="pt-6">
            <div className="flex items-start justify-between">
              <div>
                ...
                <p className="text-3xl font-bold text-primary">${user?.balance.toFixed(2)}</p>
              </div>
            ...
            {[
              ...
              >
                ...
                <CardContent className="space-y-4">
                  ...
                  <Button
                    onClick={() => handleUpgradePlan(plan.name.toLowerCase() as any, plan.price)}
                    disabled={user?.plan === plan.name.toLowerCase() || (user?.balance || 0) < plan.price}
                    className={`w-full ${
                      user?.plan === plan.name.toLowerCase()
